import { concat, interval, timer, merge, combineLatest, zip, of, forkJoin } from 'rxjs';
...
(() =>{
        ...
        getRequest(`${SW_API}/people/1`).pipe(
            // Realizar los operadores respectivos aquí
               ...
               switchMap(res=>zip(of(res), getRequest('https://swapi.dev/api/species/1'))),
                map(([personajes, especies])=> ({personaje:personajes, raza:especies}))
